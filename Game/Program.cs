...
Armor lightArmor = new("Light armor", 20, 1);
Armor heavyArmor = new("Heavy armor", 40, 2);
...
while (player.Hitpoint > 0 && enemy.Hitpoint > 0)
{
    ...
    Console.WriteLine($"{player.Name} attacks for {player.TotalDamage - enemy.Defense} damage. {enemy.Name} have {enemy.Hitpoint} hitpoints left.");
    Console.ReadLine();
    if (enemy.Hitpoint >= 1)
    {
        enemy.DealDamage(player);
        Console.WriteLine($"{enemy.Name} attacks for {enemy.TotalDamage - player.Defense} damage. {player.Name} have {player.Hitpoint} hitpoints left.");
        Console.ReadLine();
